...
void actualizar_tipo_id(char *nombre, int tipo);
%}
...
fun_declaration
    : type_specifier ID '(' param_list ')' compound_stmt
    | type_specifier ID '(' VOID ')' compound_stmt
    | VOID ID '(' param_list ')' compound_stmt
    | VOID ID '(' VOID ')' compound_stmt
    ;

void_fun_declaration
    : VOID MAIN '(' VOID ')' compound_stmt
    ;
...
void actualizar_tipo_id(char *nombre, int tipo) {
    ...
    while (tp != NULL) {
        if (strcmp(tp->lexeme, nombre) == 0) {
            /* ¡Lo encontramos! Actualizamos su tipo */
            tp->tipo_dato = tipo;
            return;
        }
        tp = tp->next;
